...
//		//av_log(" ","h %d w %d aspect %f blank %f is_width %d\n", height, width, aspect_ratio, blank, (is->width));
...
static int video_open(VideoState *is){	
	...
	if (!screen) {
		//av_log(" ",stderr, "SDL: could not set video mode - exiting\n");
		return -1;
	}
...
static double compute_frame_delay(double frame_current_pts, VideoState *is)
{
	...
	//av_log(" ","video: delay=%0.3f actual_delay=%0.3f pts=%0.3f A-V=%f\n", delay, actual_delay, frame_current_pts, -diff);
#endif
...
static void video_refresh_timer(void *opaque)
{
	...
	if (show_status) {
		...
		if (!last_time || (cur_time - last_time) >= 500 * 1000) {
			...
			//av_log(" ","%7.2f A-V:%7.3f aq=%5dKB vq=%5dKB sq=%5dB    \r",
...
static void alloc_picture(void *opaque)
{
	...
	if(vp->bmp == NULL) {
		//av_log(" ",stderr, "Surface failed: %s\n", SDL_GetError());
		exit(-1);
	}
...
static int output_picture2(VideoState *is, AVFrame *src_frame, double pts1)
{
...
#if defined(DEBUG_SYNC) && 0
	{
		...
		if (src_frame->pict_type == FF_B_TYPE)
			...
			ftype = 'P';
		//av_log(" ","frame_type=%c clock=%0.3f pts=%0.3f\n",
			   ftype, pts, pts1);
	}
#endif
...
#if 0
				//av_log(" ","diff=%f adiff=%f sample_diff=%d apts=%0.3f vpts=%0.3f %f\n",
	...
	for(;;) {
		...
		while (pkt_temp->size > 0) {
			...
			if (dec->sample_fmt != is->audio_src_fmt) {
				...
				if (!is->reformat_ctx) {
					//av_log(" ",stderr, "Cannot convert %s sample format to %s sample format\n",
			...
			if (is->reformat_ctx) {
				...
				if (av_audio_convert(is->reformat_ctx, obuf, ostride, ibuf, istride, len)<0) {
					//av_log(" ","av_audio_convert() failed\n");
					break;
				}
			...
			*pts_ptr = pts;
...
#if defined(DEBUG_SYNC)
			{
				...
				//av_log(" ","audio: delay=%0.3f clock=%0.3f pts=%0.3f\n",
...
static int stream_component_open(VideoState *is, int stream_index)
{
	...
	if (enc->codec_type == AVMEDIA_TYPE_AUDIO) {
		...
		if (SDL_OpenAudio(&wanted_spec, &spec) < 0) {
			//av_log(" ","SDL_OpenAudio: error");
			return -1;
		}
		is->audio_hw_buf_size = spec.size;
		is->audio_src_fmt= SAMPLE_FMT_S16;
	}
	//av_log(" ","checking thread count");
...
static int decode_thread(void *arg)
{
	...
	if (err < 0) {
		//av_log(" ",stderr, "%s: could not find codec parameters\n", is->filename);
		ret = -1;
		goto fail;
	}
	if(ic->pb)
	...
	if (start_time != AV_NOPTS_VALUE) {
		...
		if (ret < 0) {
			//av_log(" ",stderr, "%s: could not seek to position %0.3f\n",
	...
	if (subtitle_index >= 0) {
		//av_log(" ","opening stream subtitle\n");
		stream_component_open(is, subtitle_index);
	}
	
	if (is->video_stream < 0 && is->audio_stream < 0) {
		//av_log(" ",stderr, "%s: could not open codecs here \n", is->filename);
		ret = -1;
		goto fail;
	}
	...
	for(;;) {
		...
		if (is->seek_req) {
			...
			if (ret < 0) {
				//av_log(" ",stderr, "%s: error while seeking\n", is->ic->filename);
			}else{
	...
	while (!done) {
		...
		switch(event.type) {
			...
			case SDL_MOUSEBUTTONDOWN:
				...
				//av_log(" ","But I went to mousedown instead");
				do_exit(&done);
				/*
				if (cur_stream) {
					int ns, hh, mm, ss;
					...
					mm = (ns%3600)/60;
					ss = (ns%60);
					//av_log(" ",stderr, "Seek to %2.0f%% (%2d:%02d:%02d) of total duration (%2d:%02d:%02d)       \n", frac*100,
...
static void opt_format(const char *arg)
{
	...
	if (!file_iformat) {
		//av_log(" ",stderr, "Unknown input format: %s\n", arg);
		exit(1);
	}
}
...
{
	frame_pix_fmt = opt_default("pixel_format", arg);
...
static int opt_sync(const char *opt, const char *arg)
{
	if (!strcmp(arg, "audio"))
		...
		//av_log(" ",stderr, "Unknown value for %s: %s\n", opt, arg);
		exit(1);
	}
	return 0;
}
...
#if !HAVE_THREADS
	//av_log(" ",stderr, "Warning: not compiled with thread support, using thread emulation\n");
#endif
	return 0;
}
...
//	argv[14] = "rtsp://video.bouygtel.fr/50400/tf1.sdp";
	...
	*/
	...
	//av_log(" ","tab selected %s\n" ,argv[0]);
	//av_log(" ","url selected %s\n" ,argv[14]);
	...
	if (!input_filename) {
		//av_log(" ",stderr, "An input file must be specified\n");
		exit(1);
	}
...
#endif
	if (SDL_Init (flags)) {
		//av_log(" ",stderr, "Could not initialize SDL - %s\n", SDL_GetError());
		exit(1);
	}
	
	//av_log(" ","Value of glready is %d ", glReady);
...
int initGl(void) {
	...
	glEnable(GL_DITHER);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not enable dither support: %d", err);
	glEnable(GL_TEXTURE_2D);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not enable textures: %d", err);
		
	glGenFramebuffersOES(1, &framebuffer);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not generate frame buffer: %d", err);	
	glBindFramebufferOES(GL_FRAMEBUFFER_OES, framebuffer);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not bind frame buffer: %d", err);
	glFramebufferRenderbufferOES(GL_FRAMEBUFFER_OES, GL_COLOR_ATTACHMENT0_OES, GL_RENDERBUFFER_OES, window);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not bind framebuffer: %d", err);
	
	glBindTexture(GL_TEXTURE_2D, texture);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not bind texture: %d", err);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not set texture minimization filter: %d", err);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not set texture magnification filter: %d", err);
	glEnableClientState(GL_VERTEX_ARRAY);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not enable vertex arrays: %d", err);
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not enable texture vertex arrays: %d", err);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not enable texture replacement mode: %d", err);
	
	glViewport( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not set viewport: %d", err);
	glScissor(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not set scissors: %d", err);
	
	glMatrixMode(GL_PROJECTION);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not enable projection matrix mode: %d", err);
	
	glScalef( SSCALE_X, SSCALE_Y, 0);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not set Scalef: %d", err);
	...
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not set view matrix: %d", err);
	
	glMatrixMode(GL_MODELVIEW);
	//if (err = glGetError())
		//av_log(" ",stderr,"Error: Could not set model view matrix mode: %d", err);
	glLoadIdentity();
	//if (err = glGetError())
		////(" ",stderr,"Error: Could not enable projection matrix mode: %d", err);
